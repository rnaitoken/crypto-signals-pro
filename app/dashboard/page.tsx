...
import { TrendingUp, TrendingDown, BarChart2, Star, Wallet, Users, Activity } from 'lucide-react'
...
export default function DashboardPage() {
  ...
  const handleCopy = () => {
    navigator.clipboard.writeText('https://crypto-signals-web-eight.vercel.app?ref=Synapbot')
    setCopied(true)
    ...
    <div className="min-h-screen bg-[#030712] text-white">
      ...
      <main className="relative max-w-7xl mx-auto px-6 py-10">
        <div className="flex items-center justify-between mb-10">
          <div>
            <h1 className="font-display text-3xl md:text-4xl font-black">{tx.controlPanel}</h1>
            <p className="text-gray-500 mt-1">{tx.activitySummary}</p>
          </div>
          <select value={period} onChange={e => setPeriod(e.target.value)} className="bg-white/5 border border-white/10 text-white text-sm px-4 py-2 rounded-lg cursor-pointer">
            <option value="7d">{tx.last7days}</option>
            <option value="30d">{tx.last30days}</option>
            <option value="90d">{tx.last90days}</option>
          </select>
        </div>

        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-10">
          {[
            { label: tx.totalProfit, value: `+${totalProfit.toFixed(2)}%`, icon: <TrendingUp className="w-5 h-5" />, color: 'text-[#00ff88]', bg: 'bg-[#00ff88]/10' },
            { label: tx.winRate, value: `${Math.round(wins/mockSignals.length*100)}%`, icon: <BarChart2 className="w-5 h-5" />, color: 'text-[#00ff88]', bg: 'bg-[#00ff88]/10' },
            { label: tx.signalsWon, value: `${wins}/${mockSignals.length}`, icon: <Star className="w-5 h-5" />, color: 'text-[#00d4ff]', bg: 'bg-[#00d4ff]/10' },
            { label: tx.wallet, value: tx.bybit, icon: <Wallet className="w-5 h-5" />, color: 'text-purple-400', bg: 'bg-purple-500/10' },
          ].map((stat, i) => (
        ...
        <div className="glass rounded-xl p-6 mb-6 border border-white/5">
          <div className="flex items-center justify-between mb-6">
            <h2 className="font-display font-bold text-lg">{tx.profitability}</h2>
            <span className="text-xs text-[#00ff88] bg-[#00ff88]/10 px-3 py-1 rounded-full">+{totalProfit.toFixed(2)}% {tx.thisMonth}</span>
          </div>
        ...
        <div className="glass rounded-xl overflow-hidden border border-white/5">
          <div className="px-6 py-4 border-b border-white/5 flex items-center justify-between">
            <h2 className="font-display font-bold">{tx.mySignals}</h2>
            <span className="text-xs text-gray-500">{mockSignals.length} {tx.signals}</span>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-white/5">
                <tr className="text-left text-xs text-gray-500 uppercase tracking-wider">
                  <th className="px-6 py-3">{tx.date}</th>
                  <th className="px-6 py-3">{tx.pair}</th>
                  <th className="px-6 py-3">{tx.direction}</th>
                  <th className="px-6 py-3">{tx.entry}</th>
                  <th className="px-6 py-3">{tx.exit}</th>
                  <th className="px-6 py-3">{tx.result}</th>
                </tr>
              </thead>
              <tbody>
                {mockSignals.map(s => (
                  <tr key={s.id} className="border-t border-white/5 hover:bg-white/5 transition">
                    ...
                    <td className="px-6 py-4">
                      <span className={`text-xs px-2.5 py-1 rounded-full font-medium ${s.direction === 'LONG' ? 'bg-[#00ff88]/10 text-[#00ff88]' : 'bg-red-500/10 text-red-400'}`}>
                        ...
                        {s.direction === 'LONG' ? tx.long : tx.short}
                      </span>
                    </td>
          ...
          <div className="flex items-start justify-between flex-wrap gap-4">
            <div>
              <div className="flex items-center gap-2 mb-2">
                ...
                <h3 className="font-display font-bold">{tx.affiliateProgram}</h3>
              </div>
              <p className="text-sm text-gray-400">{tx.shareEarn}</p>
            </div>
            <div className="flex items-center gap-2 w-full md:w-auto">
              ...
              <button onClick={handleCopy} className="bg-[#00ff88] text-[#030712] px-4 py-2 rounded-lg text-sm font-bold hover:brightness-110 transition shrink-0">
